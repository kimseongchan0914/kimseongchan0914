...
**Language**
`Kotlin` `Java` `Go` `TypeScript` `JavaScript(Nuxt)` `Elixir`

**Framework**
`Spring Boot` `Spring Cloud Gateway` `Eureka` `OpenFeign` `Vert.x` `NestJS` `Nuxt.js` `Gin` `Chi` `Phoenix` `LiveKit`

**Infra / Data**
`MySQL` `MongoDB` `Redis` `Elasticsearch` `Flyway` `Kafka` `Docker` `Vault` `Prometheus` `Grafana`
